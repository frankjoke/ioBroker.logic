...
//@ts-disable TS80006
//@js-disable TS80006
const acore = require("@iobroker/adapter-core");
const Sequence = require("./lib/sequence");
const CacheP = require("./lib/cachep");
const HrTime = require("./lib/hrtime");
// you have to call the adapter function and pass a options object
// name has to be set and has to be equal to adapters folder name and main file name excluding extension
// adapter will be restarted automatically every time as the configuration changed, e.g system.adapter.template.0
let adapter, aoptions, aname, amain;
/*
...
class MyAdapter extends acore.Adapter {
	...
	static init(amodule, options, ori_main) {
		...
		if (amodule && amodule.parent) {
			amodule.exports = (options) => (adapter = new MyAdapter(options));
		} else {
			adapter = new MyAdapter(aoptions);
		}
	}
...
*/
	...
	static changeState(id, value, ack, always) {
		...
		const ts = typeof ack === "number" ? ack : undefined;
		...
		if (ts) stn.ts = ts;
		return (states[id] ? Promise.resolve(states[id]) : this.getState(id).then(st => states[st] = st, () => null))
	...
	static async myGetState(id) {
		...
		if (!nid)
			nid = id;
		const s = await this.getForeignState(nid);
		return states[nid] = s;
	}
